...
function model_tests:test_map()
	local map = _g_table_utils.map(testbed, function (val)
		return val + 10
	end)
...
function model_tests:test_reduce()
	local reduce = _g_table_utils.reduce(testbed, function(val, key, memo) 
		return val + memo
	end, 0)
...
function model_tests:test_find()
	local find = _g_table_utils.find(testbed, function(val)
		return val == 7
	end)
	assert_equal(find, 7)
end
...
function model_tests:test_filter()
	local filter = _g_table_utils.filter(testbed, function (val)
		return val > 5
	end)
...
function model_tests:test_max()
	local max = _g_table_utils.max(testbed)
	assert_equal(max, 10)
end
